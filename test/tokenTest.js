...
contract('StandardToken', function ([ contractOwner, owner, recipient, anotherAccount]) {
  ...
  beforeEach(async function () {
    this.token = await token2GB.new(trezorAccount);
  });
  ...
  describe('total supply', function () {
    it('returns the total amount of tokens', async function () {
      ...
      console.log('seedBalance: ', seedBalance);

      assert.equal(totalSupply, seedBalance);
    });
  });
  ...
  describe('balanceOf', function () {
    ...
    describe('when the requested account has some tokens', function () {
      it('returns the total amount of tokens', async function () {
        const balance = await this.token.balanceOf(trezorAccount);

        assert.equal(balance, seedBalance);
      });
    });
  });
